...
class Solver:
    ...
    Functions:
    - solve: Solves the complete system. That means all Fluid mass flows 
    ...
    - solve_flows: Solves only the Fluid mass flows of the system. That means
    ...
    Attributes:
    - system (System): Instance of the class System which is simulated.
    """
    ...
    def _calculate_mass_flows(self, time, dt):
        ...
        f = np.ones(self.system.N_boxes) # scaling factor for sinks of each box
        ...
        dimless_dt = ((self.system.flows[0].units/m_units)*dt).to_base_units().magnitude
    ...
    def _calculate_changes_of_variable(self, time, dt, variable, f_flow):
        ...
        var_conc, var_conc_units = self.system.get_box_variable_concentration_vector(variable)
        ...
        tt_flows = [flow for flow in self.system.flows if flow.tracer_transport==True]  # Tracer-Transport Flows
        ...
        mass_flow_s = self.system.get_flow_sink_vector(time, tt_flows_with_variables)*f_flow
        mass_flow_q = self.system.get_flow_source_vector(time, tt_flows)*f_flow

        # all of these vectors have units of kg/s
        flow_s = mass_flow_s * var_conc
        flow_q = mass_flow_q
        p_s, p_s_units = self.system.get_process_sink_vector_of_variable(time, variable)
        p_q, p_q_units = self.system.get_process_source_vector_of_variable(time, variable)
        ...
        f_s, f_s_units = self.system.get_flux_sink_vector_of_variable(time, variable)
        f_q, f_q_units = self.system.get_flux_source_vector_of_variable(time, variable)
    ...
    def solve(self, total_integration_time, dt, debug=True):
        ...
        for i in range(N_time_steps):
            ...
            for var_name, var in self.system.variables.items():
                ...
                var_changes[var_name] = self._calculate_changes_of_variable(time, dt, var, A)
            ...
            for box_name, box in self.system.boxes.items():
                for var_name, var in box.variables.items():
                    dprint('1111self.boxes[{}].variables[{}].mass: '.format(box_name, var_name), self.system.boxes[box_name].variables[var_name].mass)
            dprint('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
            ...
            for box_name, box in self.system.boxes.items():
                ...
                for var_name, var in box.variables.items():
                    ...
                    dprint('@@@@ : ', var_changes[var_name][box.ID])
                    dprint('@@@@ : ', var_units[var_name])
                    dprint('@@@@ : ', self.system.boxes[box_name].variables[var_name].mass)


                    new_var_mass = var_changes[var_name][box.ID] * var_units[var_name] + self.system.boxes[box_name].variables[var_name].mass
                    ...
                    sol.ts[box_name][var_name].append(self.system.boxes[box_name].variables[var_name].mass)


                    self.system.mm.loc[box_name, var_name] = new_var_mass
                    if box.fluid.mass.magnitude > 0:
                        self.system.cm.loc[box_name, var_name] = new_var_mass / box.fluid.mass
                    else: 
                        self.system.cm.loc[box_name, var_name] = 0

                    dprint('$$$$$$2222self.boxes[{}].variables[{}].mass: '.format(box_name, var_name), self.system.boxes[box_name].variables[var_name].mass)
            time += dt
